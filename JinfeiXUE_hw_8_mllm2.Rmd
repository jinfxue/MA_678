...
log_1 <- glm(match~attr_o +sinc_o +intel_o +fun_o +amb_o +shar_o,data=dating,family=binomial)
summary(log_1)
coefplot(log_1)
```
...
```{r,warning=FALSE}
log_mix <- lmer(match~gender+scale(attr_o) +scale(sinc_o) +scale(intel_o) +scale(fun_o) +scale(amb_o) +scale(shar_o)+(1|iid),data=dating,family=binomial(link="logit"))
summary(log_mix)
#ranef(log_mix)
```
...
```{r}
log_mix2 <- glmer(match~gender+scale(attr_o) +scale(sinc_o) +scale(intel_o) +scale(fun_o) +scale(amb_o) +scale(shar_o)+(1|iid)+(1|pid),data=dating,family=binomial)
summary(log_mix2)
a_iid <- data.frame(ranef(log_mix2))[1:5,4]
a_pid <- data.frame(ranef(log_mix2))[552:556,4]
a_iid
a_pid
```
...
vary_slope <- glmer(match~(1+attr_o+sinc_o+intel_o+fun_o+amb_o+shar_o|iid) + attr_o + sinc_o + intel_o + fun_o + amb_o + shar_o,data=dating,family=binomial)
```
...
```{r}
anova(vary_slope,log_1,no_pooling)
```
