...
set.seed(123)
index <- data_frame(User_ID = sample(unique(data$User_ID), 500, replace = FALSE)) %>%
  arrange(User_ID)
...
boxplot(bf$Purchase, main="Purchase Amount", boxwex=0.1)
outlier_values <- boxplot.stats(bf$Purchase)$out  # outlier values
bf$Purchase[which(bf$Purchase %in% outlier_values)]=median(bf$Purchase)
...
### 2.2.1 Clean Test Data

```{r}
sapply(test, function(x) sum(is.na(x)))
# Because NAs only happen in product category variables and it represents the client didn't buy any products in the category, I replace NA with zero.
test[is.na(test)] <- 0
sum(is.na(test)) #check

# transfer occupation to factor
test$Occupation <- as.factor(test$Occupation)
```

### 2.2.2 Sample (According to the Train Dataset Sample)

```{r, message=FALSE}
bf_test <- inner_join(test, index) # test data sample
...
### 4.1.3 Linear regression model (with interaction)
...
```{r, message=FALSE, warning=FALSE}
# Fit the full model
r4_full <- polr(purchase_level ~ Gender + Age + Occupation + City_Category + 
           Stay_In_Current_City_Years + Marital_Status + Product_Category_1 +
           Product_Category_2+Product_Category_3, data = bf)

# Stepwise regression model(select the model which has the smallest AIC)
library(MASS)
r4 <- stepAIC(r4_full, direction = "both", trace = FALSE)
summary(r4)
...
### 4.1.5 Multilevel cumulative logit model (vary by intercept)
